...
public class TrinketScreenManager {
	...
	public static void update(float mouseX, float mouseY) {
		...
		if (group == null) {
			for (SlotGroup g : TrinketsApi.getPlayerSlots(Minecraft.getInstance().player).values()) {
	...
	public static void drawGroup(GuiComponent helper, PoseStack matrices, SlotGroup group, SlotType type) {
		TrinketPlayerScreenHandler handler = currentScreen.trinkets$getHandler();
		...
		if (slotsWidth > 1 || type != null) {
			helper.blit(matrices, x, y, 0, 0, 4, 26);

			for (int i = 0; i < slotsWidth; i++) {
				helper.blit(matrices, x + i * 18 + 4, y, 4, 0, 18, 26);
			}

			helper.blit(matrices, x + slotsWidth * 18 + 4, y, 22, 0, 4, 26);
			for (int s = 0; s < slotHeights.size() && s < slotTypes.size(); s++) {
				...
				if (height > 1) {
					...
					if (height > 2) {
						helper.blit(matrices, slotX, y - top * 18, 0, 0, 26, 4);
					}

					for (int i = 1; i < top + 1; i++) {
						helper.blit(matrices, slotX, y - i * 18 + 4, 0, 4, 26, 18);
					}
					...
					helper.blit(matrices, slotX, y + 18 + bottom * 18 + 4, 0, 22, 26, 4);
				}
			}
			...
			for (int s = 0; s < slotHeights.size(); s++) {
				...
				int top = (height - 1) / 2;
				int bottom = height / 2;
				helper.blit(matrices, slotX, y - top * 18 + 1, 4, 1, 16, 3);
				helper.blit(matrices, slotX, y + (bottom + 1) * 18 + 4, 4, 22, 16, 3);
			}

			// Because pre-existing slots are not part of the slotHeights list
			if (group.getSlotId() != -1) {
				helper.blit(matrices, r.getX() + 1, y + 1, 4, 1, 16, 3);
				helper.blit(matrices, r.getX() + 1, y + 22, 4, 22, 16, 3);
			}
		} else {
			helper.blit(matrices, x + 4, y + 4, 4, 4, 18, 18);
		}
	...
	public static void drawActiveGroup(GuiComponent helper, PoseStack matrices) {
	...
	public static void drawExtraGroups(GuiComponent helper, PoseStack matrices) {
		TrinketPlayerScreenHandler handler = currentScreen.trinkets$getHandler();
		int x = currentScreen.trinkets$getX();
		int y = currentScreen.trinkets$getY();
		...
		RenderSystem.setShaderTexture(0, MORE_SLOTS);
		helper.blit(matrices, x + 3, y,      7, 26, 1, 7);
		// Repeated tops and bottoms
		for (int i = 0; i < width; i++) {
			helper.blit(matrices, x - 15 - 18 * i, y,      7, 26, 18, 7);
			helper.blit(matrices, x - 15 - 18 * i, y + 79, 7, 51, 18, 7);
		}
		// Top and bottom
		helper.blit(matrices, x - 15 - 18 * width, y,                   7, 26, 18, 7);
		helper.blit(matrices, x - 15 - 18 * width, y + 7 + 18 * height, 7, 51, 18, 7);
		// Corners
		helper.blit(matrices, x - 22 - 18 * width, y,                   0, 26, 7, 7);
		helper.blit(matrices, x - 22 - 18 * width, y + 7 + 18 * height, 0, 51, 7, 7);
		// Outer sides
		for (int i = 0; i < height; i++) {
			helper.blit(matrices, x - 22 - 18 * width, y + 7 + 18 * i, 0, 34, 7, 18);
		}
		// Inner sides
		if (width > 0) {
			for (int i = height; i < 4; i++) {
				helper.blit(matrices, x - 4 - 18 * width, y + 7 + 18 * i, 0, 34, 7, 18);
			}
		}
		if (width > 0 && height < 4) {
			// Bottom corner
			helper.blit(matrices, x - 4 - 18 * width, y + 79, 0, 51, 7, 7);
			// Inner corner
			helper.blit(matrices, x - 4 - 18 * width, y + 7 + 18 * height, 0, 58, 7, 7);
		}
		if (width > 0 || height == 4) {
			// Inner corner
			helper.blit(matrices, x, y + 79, 0, 58, 3, 7);
		}
	}
