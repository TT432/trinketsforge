    ...
    public static void onTrinketBroken(ItemStack stack, SlotReference ref, LivingEntity entity) {
        if (!entity.level.isClientSide) {
            ...
            if (entity instanceof ServerPlayer player) {
                TrinketsNetHandler.sendToClient(player, TrinketsNetwork.BREAK, buf);
            }

            Players.tracking(entity).forEach(watcher ->
                    TrinketsNetHandler.sendToClient(watcher, TrinketsNetwork.BREAK, buf));
        }
    }
